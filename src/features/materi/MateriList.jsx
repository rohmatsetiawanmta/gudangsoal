...
export default function MateriList() {
  ...
  // cascade: jenjang → subjenjang
  useEffect(() => {
    setSelSubjenjang(null);
    setSelMapel(null);
    setSelTopik(null);
    setSelSubtopik(null);
    setSubjenjangList([]);
    ...
    api
      .get(`/browse/subjenjang-list?jenjang_id=${selJenjang}`)
      .then((data) => setSubjenjangList(Array.isArray(data) ? data : []))
      .catch(() => {})
      .finally(() => setLoadingSubjenjang(false));
  }, [selJenjang]);

  // cascade: subjenjang → mapel
  useEffect(() => {
    setSelMapel(null);
    setSelTopik(null);
    setSelSubtopik(null);
    ...
    api
      .get(`/browse/mapel-list?subjenjang_id=${selSubjenjang}`)
  ...
  useEffect(() => {
    ...
    api
      .get(`/browse/topik-list?mapel_id=${selMapel}`)
      .then((data) => setTopikList(Array.isArray(data) ? data : []))
      .catch(() => {})
      .finally(() => setLoadingTopik(false));
  }, [selMapel]);

  // cascade: topik → subtopik chips
  useEffect(() => {
    setSelSubtopik(null);
    setSubtopikList([]);
    if (!selTopik) return;
    ...
    api
      .get(`/browse/subtopik-list?topik_id=${selTopik}`)
      .then((data) => setSubtopikList(Array.isArray(data) ? data : []))
      .catch(() => {})
      .finally(() => setLoadingSubtopik(false));
  }, [selTopik]);
  ...
  // Only fetch when subtopik is selected OR search is active
  const showResults = !!selSubtopik || !!debouncedSearch;

  // build filter param for list API — most specific wins
  const filterParam = selSubtopik
    ? `subtopik_id=${selSubtopik}`
    : selTopik
    ? `topik_id=${selTopik}`
    : selMapel
    ? `mapel_id=${selMapel}`
    : selSubjenjang
    ? `subjenjang_id=${selSubjenjang}`
    : selJenjang
    ? `jenjang_id=${selJenjang}`
    : "";
      ...
      >
        ...
        <div
          style={{
            borderRadius: "18px",
            background:
              "linear-gradient(135deg, #0f0e17 0%, #1a1830 55%, #0a1c1a 100%)",
        ...
        >
          ...
          >
            <FilterSelect
              label="Semua Jenjang"
              value={selJenjang ?? ""}
              onChange={setSelJenjang}
              options={jenjangList}
              disabled={false}
            />
            <FilterSelect
              label={loadingSubjenjang ? "Memuat..." : "Semua Kelas"}
              value={selSubjenjang ?? ""}
              onChange={setSelSubjenjang}
              options={subjenjangList}
              disabled={!selJenjang || loadingSubjenjang}
            />
            <FilterSelect
              label={loadingMapel ? "Memuat..." : "Semua Mapel"}
              value={selMapel ?? ""}
              onChange={setSelMapel}
              options={mapelList}
              disabled={!selSubjenjang || loadingMapel}
            />
            <FilterSelect
              label={loadingTopik ? "Memuat..." : "Semua Topik"}
              value={selTopik ?? ""}
              onChange={setSelTopik}
              options={topikList}
              disabled={!selMapel || loadingTopik}
            />
          </div>
              ...
              {loadingSubtopik ? (
                ...
                subtopikList.map((st) => {
                  ...
                  return (
                    <button
                      key={st.id}
                      onClick={() => setSelSubtopik(active ? null : st.id)}
                      style={{
                        padding: "5px 12px",
                        borderRadius: "99px",
                        fontSize: "12.5px",
                        ...
                        border: active
                          ? "1px solid #1a8a6e"
                          : "1px solid #e2ddd5",
                        background: active ? "#e4f5f0" : "white",
                        color: active ? "#1a8a6e" : "#6b6860",
                        cursor: "pointer",
                        fontFamily: "inherit",
                        transition: "all .12s",
                      }}
          ...
          {hasFilter && (
            ...
            >
              ...
              >
                ...
                {debouncedSearch && (
                  <span style={{ fontWeight: "600", color: "#0f0e17" }}>
                    "{debouncedSearch}"
                  </span>
                )}
              </div>
        ...
        {/* List */}
        {!showResults ? (
          <div style={{ background: "white", borderRadius: "16px", border: "1px solid #e2ddd5", padding: "48px 32px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: "15px", fontWeight: "700", color: "#0f0e17", marginBottom: "6px" }}>Pilih subtopik terlebih dahulu</div>
            <p style={{ fontSize: "13px", color: "#6b6860", margin: 0, lineHeight: 1.6 }}>
          ...
          >
            ...
            <p style={{ fontSize: "13px", color: "#6b6860", margin: 0 }}>
              {hasFilter
                ...
                : "Konten materi sedang disiapkan."}
            </p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "12px", color: "#b4b2a9" }}>
                      {[item.jenjang, item.mapel, item.topik, item.subtopik]
                        .filter(Boolean)
                        .join(" › ")}
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", gap: "6px" }}>
              ...
              <button
                ...
                style={{
                  ...
                  alignItems: "center",
                  justifyContent: "center",
                  color: page === totalPages ? "#d4d0c8" : "#6b6860",
                }}
              >
