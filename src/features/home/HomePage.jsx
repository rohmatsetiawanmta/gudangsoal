...
import {
  ...
  Zap, Flame, Dumbbell, GraduationCap,
...
export default function HomePage() {
  ...
  const QUICK_ACTIONS = [
    {
      label: "Direktori Soal",
      desc:  "Jelajahi soal berdasarkan jenjang & topik",
      icon:  BookOpen,
      color: "#2563eb",
    ...
    },
    {
      ...
      desc:  "Teori & rumus per subtopik",
      icon:  GraduationCap,
      color: "#1a8a6e",
      bg:    "rgba(26,138,110,.12)",
      to:    "/materi",
    },
  ];
    ...
    <div style={{ minHeight: "100vh", background: "var(--gs-bg)" }}>
      ...
      <main style={{ maxWidth: "960px", margin: "0 auto", padding: isMobile ? "20px 16px 48px" : "36px 32px 64px" }}>
        ...
        <div style={{
          ...
          <div style={{ position: "absolute", right: isMobile ? "-10px" : "32px", top: "50%", transform: "translateY(-50%)", opacity: 0.04, pointerEvents: "none", color: "white" }}>
          ...
          <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", flexDirection: isMobile ? "column" : "row", gap: "20px" }}>
            <div>
              <div style={{ fontSize: "11px", fontWeight: "600", color: "rgba(255,255,255,.4)", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: "6px" }}>
                {getGreeting()}
              </div>
        ...
        <div style={{
          ...
          {STAT_CARDS.map(({ label, value, icon: Icon, color, bg }) => (
            <div key={label} style={{
              ...
              <div>
                <div style={{ fontSize: isMobile ? "18px" : "20px", fontWeight: "800", color: "var(--gs-text)", letterSpacing: "-0.5px", lineHeight: 1.1 }}>
                  {loadingRiwayat ? "—" : value}
                </div>
                <div style={{ fontSize: "11px", color: "var(--gs-text-hint)", fontWeight: "500", marginTop: "2px" }}>{label}</div>
              </div>
            </div>
          ))}
        </div>
          ...
          {QUICK_ACTIONS.map(({ label, desc, icon: Icon, color, bg, to }) => (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ width: "44px", height: "44px", borderRadius: "13px", background: bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Icon size={22} color={color} />
                </div>
                <div style={{ width: "28px", height: "28px", borderRadius: "8px", border: "1px solid var(--gs-border)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <ChevronRight size={13} color="var(--gs-text-hint)" />
                </div>
              </div>
              <div>
                <div style={{ fontSize: "15px", fontWeight: "700", color: "var(--gs-text)", marginBottom: "4px" }}>{label}</div>
                <div style={{ fontSize: "12.5px", color: "var(--gs-text-muted)", lineHeight: "1.5" }}>{desc}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ width: "3px", height: "16px", borderRadius: "2px", background: "#e84c2b", flexShrink: 0 }} />
            <h2 style={{ fontSize: "15px", fontWeight: "700", color: "var(--gs-text)", margin: 0 }}>Sesi Terakhir</h2>
          </div>
          {riwayat.length > 0 && (
            <button onClick={() => navigate("/profile?tab=riwayat")} style={{ fontSize: "13px", color: "#e84c2b", fontWeight: "600", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit" }}>
              Lihat semua
            </button>
          )}
        </div>
        ...
        {loadingRiwayat ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} style={{ height: "60px", borderRadius: "12px", background: "var(--gs-border)", opacity: 0.5, animation: "pulse 1.5s infinite" }} />
            ))}
          </div>
        ) : riwayat.length === 0 ? (
          <div style={{ background: "var(--gs-surface)", borderRadius: "14px", border: "1px solid var(--gs-border)", padding: "40px", textAlign: "center" }}>
            <div style={{ width: "46px", height: "46px", borderRadius: "13px", background: "var(--gs-hover)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px" }}>
              <Dumbbell size={22} color="var(--gs-text-hint)" />
            </div>
            <p style={{ fontSize: "15px", fontWeight: "600", color: "var(--gs-text)", margin: "0 0 4px" }}>Belum ada sesi latihan</p>
            <p style={{ fontSize: "13px", color: "var(--gs-text-muted)", margin: "0 0 16px" }}>Pilih jenjang di direktori untuk mulai latihan pertamamu!</p>
            <button onClick={() => navigate("/browse")} style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "9px 18px", borderRadius: "10px", border: "none", background: "#e84c2b", color: "white", fontSize: "13px", fontWeight: "600", cursor: "pointer", fontFamily: "inherit" }}>
              <BookOpen size={13} /> Mulai Latihan
            </button>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {riwayat.map((r, i) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: "11.5px", color: "var(--gs-text-hint)", marginTop: "3px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {r.jenjang} · {r.mapel} · {r.subtopik}
                  </div>
                </div>
                {!isMobile && (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "3px", flexShrink: 0 }}>
                    <span style={{ fontSize: "11px", fontWeight: "700", color: "var(--gs-text-hint)", fontFamily: "monospace" }}>{r.kode}</span>
